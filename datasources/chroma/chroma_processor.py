...
class ChromaProcessor(DataProcessor):
    def __init__(self, builder: EngineAI, system_prompt=None):
        """
        Constructor for the Chroma class.
    ...
    def init_store(self, indexes: list[IndexItem] = None):
        ...
        if indexes is None or len(indexes) == 0:
            indexes = self.db.list_collections()
    ...
    def setup_query_engine(self, name, description=None):
    ...
    def ingest_data(self, index_name, storage: AzureStorage | S3Storage | LocalStorage):
        chroma_collection = self.db.get_or_create_collection(index_name)
        vector_store = ChromaVectorStore(chroma_collection=chroma_collection)
